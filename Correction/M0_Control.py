...
def process(args):
    print('Multicore!')
    (context, pre_M5, pre_M6, pre_M8, pre_M10, weight, lang) = args
    # # 评测语法模块F0.5用
    ...
    nlp_depend = []  # nlp依存
    nlp_pos = []  # nlp标注
    print("forqian",time.time() - start_time)
    for sent in spell_corrected_context:
        nlp_depend.append(nlp.dependency_parse(sent))
        nlp_pos.append(nlp.pos_tag(sent))
    print("forhou",time.time() - start_time)
    print("=============M6 Classifying Sentence Pattern=============")
    print(time.time() - start_time)
    # print(level)
    # print(spell_corrected_context)
    if lang == 'cn':
        M6_res = M6_eva.eva_cn(spell_corrected_context, nlp_depend, nlp_pos, fw_pre, fw_depend, fw_pos, level)
    elif lang == 'en':
        M6_res = M6_eva.eva_en(spell_corrected_context, nlp_depend, nlp_pos, fw_pre, fw_depend, fw_pos, level)
    ...
    score3_3, eva3_3 = M7_Vocabulary_Count.Count(fw_num, my_num, lang)
    score3_0, eva3_0 = M7_Vocabulary_Richness.Richness(dict_occur, my_num, lang)
    score3_1, eva3_1, eva_list_3_1 = M7_Vocabulary_Replace0.Replace0(processraw, lang)
    score3_2, eva3_2, eva_list = M7_Vocabulary_Replace1.Replace1(process, processraw, int(level)-1, lang)
    ...
    M8_score, M8_eva, M8_level, base64_stu,base64_fw = M8_Topic.call_on(st, fw_pre, pre_M8, lang)
    ...
    if lang == 'cn':
        M5_res = M5.M5_Locate_Classify_cn(wrongsent, pos_word, mis_spedded_word, sgted_word,
                                          lines_in_m2, lines_in_corrected, lines_in_source, my_num)
    elif lang == 'en':
        M5_res = M5.M5_Locate_Classify_en(wrongsent, pos_word, mis_spedded_word, sgted_word,
                                          lines_in_m2, lines_in_corrected, lines_in_source, my_num)
    ...
    M9_res = M9_Htmlautomaker.automaker(gui_left_content, suggestion, statistic, spell_score, grammar_score, M6_res, M7_score, M7_eva, M8_score, M8_eva, eva_list, context, weight, base64_stu, base64_fw, M10_res, word_seped, lang)
    ...
    print("M0_control,pid and ppid", os.getpid(), os.getppid())
    del gui_left_content,fw,fw_pre,fw_depend,fw_pos,level
    del my_num,stn_seped,spell_corrected_context,nlp_depend,nlp_pos
    del M5_res,lines_in_source,lines_in_corrected
    del M6_res,stat_sentence_passratio,stat_sentence_subjratio
    del stat_sentence_pararatio,stat_sentence_compratio,stat_sentence_simpratio
    del word_seped,dict_occur,process, processraw,score3_3, eva3_3,score3_0, eva3_0,score3_1, eva3_1, eva_list_3_1
    del score3_2,eva3_2,eva_list,M7_score,M7_eva
    del st,M8_score,M8_eva,M8_level,base64_stu,base64_fw,stat_topic_level
    del M3_res,wrongsent,pos_word,mis_spedded_word,sgted_word,Spell_corrected_context
    del Grammar_corrected_context,lines_in_m2,suggestion,spell_score
    del grammar_score,statistic,stat_misspell_num,stat_nounerr_num,stat_verberr_num,stat_other_num
    del phrase_string,meaning,knowledge,example,M10_res
    gc.collect()
