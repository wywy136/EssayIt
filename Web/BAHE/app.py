...
def user_info(username):
    if request.method == 'POST':
        if not request.form['title'] or not request.form['content']:
            ...
            M6_res, M7_score, M7_eva, M8_score, M8_eva = process(article.content)

            flash('Record was successfully added')
            return render_template('parallel.html', suggestion=suggestion, spell_score=spell_score,
                                   grammar_score=grammar_score, statistic=statistic, content=content,
                                   username=username, M6_res=M6_res, M7_score=M7_score, M7_eva=M7_eva,
                                   M8_score=M8_score, M8_eva=M8_eva)
